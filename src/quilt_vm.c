/* quilt_vm.c — Implementation of the 5-opcode Quilt VM, on ESP32.
 *
 * This is a direct port of /workspace/quilt-vm-c/src/quilt_vm.c.
 * The same 5 opcodes, the same struct, the same semantics.
 * The 5 opcodes are the runtime. C is the desert. The ESP32
 * is the open range.
 *
 * Differences from the host port:
 *   - <stdio.h> is still included, but you may want to swap
 *     printf for esp_log on real hardware.
 *   - No POSIX headers; realloc/calloc/strdup from <stdlib.h>
 *     and <string.h> are provided by newlib in ESP-IDF.
 *   - The VM keeps the C99 portable style: no ESP-specific
 *     types leak in here. The mesh layer above (esp_now_mesh.c)
 *     is the only place that knows it's an ESP32.
 */
...
static qvm_thing_t *qvm_thing_new(const char *name, void *value,
    ...
    if (!t->effects) { free(t->name); free(t); return NULL; }
    return t;
}

static void qvm_thing_free(qvm_thing_t *t) {
    if (!t) return;
...
int qvm_bind(qvm_t *vm, const char *name, void *value,
    ...
    if (vm->n_things >= vm->things_capacity) {
        ...
        qvm_thing_t **nt = (qvm_thing_t **)realloc(vm->things,
...
int qvm_link(qvm_t *vm, const char *a, const char *b, const char *type) {
    ...
    for (size_t i = 0; i < ta->n_link_types; i++) {
        if (strcmp(ta->link_types[i], type) == 0) {
            ...
            char **nt = (char **)realloc(ta->link_targets[i],
                (n + 2) * sizeof(char *));
            if (!nt) return -1;
            ta->link_targets[i] = nt;
    ...
    /* New type for a */
    char **lt = (char **)realloc(ta->link_types,
        (ta->n_link_types + 2) * sizeof(char *));
    if (!lt) return -1;
    ta->link_types = lt;
    char ***lts = (char ***)realloc(ta->link_targets,
        (ta->n_link_types + 2) * sizeof(char **));
    if (!lts) return -1;
    ta->link_targets = lts;
...
reverse_link: {
    ...
    for (size_t i = 0; i < tb->n_link_types; i++) {
        if (strcmp(tb->link_types[i], reverse_type) == 0) {
            ...
            char **nt = (char **)realloc(tb->link_targets[i],
                (n + 2) * sizeof(char *));
            if (!nt) return -1;
            tb->link_targets[i] = nt;
            tb->link_targets[i][n] = strdup(a);
            tb->link_targets[i][n+1] = NULL;
            return 0;
        }
    }
    char **lt2 = (char **)realloc(tb->link_types,
        (tb->n_link_types + 2) * sizeof(char *));
    if (!lt2) return -1;
    tb->link_types = lt2;
    char ***lts2 = (char ***)realloc(tb->link_targets,
        (tb->n_link_types + 2) * sizeof(char **));
    if (!lts2) return -1;
    tb->link_targets = lts2;
...
int qvm_effect(qvm_t *vm, const char *target,
    ...
    if (vm->n_pending >= vm->pending_capacity) {
        ...
        struct qvm_effect_record *np =
            (struct qvm_effect_record *)realloc(vm->pending,
    ...
    if (t->n_effects >= t->effects_capacity) {
        ...
        struct qvm_effect_record *ne =
            (struct qvm_effect_record *)realloc(t->effects,
...
void qvm_tick(qvm_t *vm, double dt) {
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        if (vm->n_events >= vm->events_capacity) {
            ...
            qvm_event_t *ne = (qvm_event_t *)realloc(vm->event_log,
                vm->events_capacity * sizeof(qvm_event_t));
            if (ne) vm->event_log = ne;
        }
        ...
        ev->target[sizeof(ev->target) - 1] = '\0';
        ev->old_value = NULL;
        ev->new_value = NULL;
        free(e.target);
    }
    ...
    tick_event.kind[sizeof(tick_event.kind) - 1] = '\0';
...
int qvm_schedule(qvm_t *vm, const char *key, qvm_scheduled_fn fn,
    ...
    if (vm->n_scheduled >= vm->scheduled_capacity) {
        ...
        qvm_scheduled_t *ns = (qvm_scheduled_t *)realloc(vm->scheduled,
...
int qvm_subscribe(qvm_t *vm, qvm_subscriber_fn fn, void *arg) {
    if (vm->n_subscribers >= vm->subscribers_capacity) {
        ...
        qvm_subscriber_fn *ns = (qvm_subscriber_fn *)realloc(vm->subscribers,
            vm->subscribers_capacity * sizeof(qvm_subscriber_fn));
        if (!ns) return -1;
        vm->subscribers = ns;
        void **nsa = (void **)realloc(vm->subscriber_args,
            vm->subscribers_capacity * sizeof(void *));
        if (!nsa) return -1;
        vm->subscriber_args = nsa;
    }
